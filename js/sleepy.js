...
function knockout(rightnow) {
	var r = '<h2>OPTIMUM WAKE TIMES ARE</h2><table id="times">'; // return string
	...
	for (var ctr = 0; ctr < 6; ctr++) { // normal sleep schedule
		...
		if (ctr == 0) {
			...
			if (min > 9) {
				r = r + '<tr><td><a href="#" rel="' + dhr + ':' + min + ap + '">' + dhr + ':' + min + ap + '</a></td></tr>';
			} else {
				r = r + '<tr><td><a href="#" rel="' + dhr + ':0' + min + ap + '">' + dhr + ':0' + min + ap + '</a></td></tr>';
			}
